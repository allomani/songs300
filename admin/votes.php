...
if ($action == "votes" ||  $action=="vote_del" ||  $action == "vote_active"  || $action=="vote_add" ){
 ...
 if($action=="vote_add"){
        db_query("insert into songs_votes_cats (title) values('".db_escape($title)."')");
        }
...
while($data = db_fetch($qr)){
  ...
  //--------------------------------
   if($action=="vote_edit_ok"){
      db_query("update songs_votes_cats set title='".db_escape($title)."' where id='$id'");

         }
  //------------------------------------------
    if ($action=="vote2_add"){
            db_query("insert into songs_votes (title,cat) values('".db_escape($title)."','".db_escape($cat)."')");
            }
  //---------------------------------------
  if($action=="vote2_del"){
  ...
  if($action=="vote2_edit_ok"){
          db_query("update songs_votes set title='".db_escape($title)."' where id='$id'");
          }
  //---------------------------------------
