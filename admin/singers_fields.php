...
//---------------------- Singers Fields ---------------------
if($action=="singers_fields" || $action=="singers_fields_edit_ok" || $action=="singers_fields_add_ok" || $action=="singers_fields_del" ||  $action=="singers_fields_enable" || $action=="singers_fields_disable"){

 if_admin("singers_fields");                                                                                                                                                                      
 
 
 if($action=="singers_fields_enable"){
    db_query("update songs_singers_fields set active=1 where id='$id'");
}

if($action=="singers_fields_disable"){
    db_query("update songs_singers_fields set active=0 where id='$id'");
}


if($action=="singers_fields_del"){
$id=intval($id);
db_query("delete from songs_singers_fields where id='$id'");
db_query("ALTER TABLE  songs_singers  DROP  `field_".$id."`");
}

if($action=="singers_fields_edit_ok"){
$id=intval($id);
if($name){
    $value=trim($value);
db_query("update songs_singers_fields set name='".db_escape($name)."',details='".db_escape($details)."',`type`='".db_escape($type)."',value='".db_escape($value,false)."',style='$style_v',ord='".intval($ord)."',enable_search='".intval($enable_search)."' where id='$id'");
    }
}

if($action=="singers_fields_add_ok"){
$id=intval($id);
if($name){
    $value=trim($value);
db_query("insert into songs_singers_fields  (name,details,`type`,value,style,ord,enable_search) values('".db_escape($name)."','".db_escape($details)."','".db_escape($type)."','".db_escape($value,false)."','$style_v','$ord','".intval($enable_search)."')");

$field_id = mysql_insert_id();

db_query("ALTER TABLE  songs_singers ADD  `field_".$field_id."` VARCHAR( 255 ) NOT NULL , ADD INDEX (  `field_".$field_id."` )");  
}
}


print "<p align=center class=title> $phrases[singers_fields]</p>

<p align=$global_align><a href='index.php?action=singers_fields_add'><img src='images/add.gif' border=0> $phrases[add_member_custom_field] </a></p>";




$qr= db_query("select * from songs_singers_fields order by ord asc");
if(db_num($qr)){
print "<center><table width=90% class=grid><tr><td>
<div id=\"singers_fields_list\">";
...
<td>".iif(!$data['active'],"<a href='index.php?action=singers_fields_enable&id=$data[id]'>$phrases[enable]</a>","<a href='index.php?action=singers_fields_disable&id=$data[id]'>$phrases[disable]</a>")."
 - <a href='index.php?action=singers_fields_edit&id=$data[id]'>$phrases[edit]</a> - <a href='index.php?action=singers_fields_del&id=$data[id]' onClick=\"return confirm('$phrases[are_you_sure]');\">$phrases[delete]</a></td></tr>
</table></div>";
...
print "</div>
 ...
 print "<script type=\"text/javascript\">
        init_sortlist('singers_fields_list','set_singers_fields_sort');
</script>";
...
//---------- Add Member Field -------------
if($action=="singers_fields_add"){
 if_admin("singers_fields");
 
print "<img src='images/arrw.gif'>&nbsp; <a href='index.php?action=singers_fields'>$phrases[singers_fields]</a> / $phrases[add_custom_field]</a><br>";

print "<center>
<p align=center class=title>$phrases[add_custom_field]</p>
<form action=index.php method=post>
<input type=hidden name=action value='singers_fields_add_ok'>
...
//---------- Edit Member Field -------------
if($action=="singers_fields_edit"){

    if_admin("singers_fields");
$id=intval($id);

$qr = db_query("select * from songs_singers_fields where id='$id'");

if(db_num($qr)){
$data = db_fetch($qr);

print "<img src='images/arrw.gif'>&nbsp; <a href='index.php?action=singers_fields'>$phrases[singers_fields]</a> / $data[name]</a><br><br>";



print "<center><form action=index.php method=post>
<input type=hidden name=action value='singers_fields_edit_ok'>
<input type=hidden name=id value='$id'>
<table width=80% class=grid>";
